export const vertexShaderWaterNight = /*glsl*/`
varying vec2 vUv;
uniform float time;
...
export const fragmentShaderWaterNight = /*glsl*/`
...
void main() {
    ...
    uv.x += cos(uv.y * 10.0 + time * 0.5) * 0.06;

    // Get the base texture color
    vec4 color = texture2D(uTexture, uv);

    // Slightly modify the color to make it look more like water
    vec3 waterColor = vec3(0.1, 0.4, 0.6); // Base water color (blueish)
    vec3 finalColor = mix(color.rgb, waterColor, 0.17); // Blend with texture
